...
import { storePending, storeResult } from "@/lib/store";
...
export async function POST(req: NextRequest) {
  ...
  storePending(requestId, pendingData);
  ...
  // Dev mode: no PayDunya keys → generate directly and redirect to success page
  if (!masterKey) {
    try {
      const apiKey = process.env.OPENAI_API_KEY ?? "";
      const copy = await generateCopy(pendingData, apiKey);
      storeResult(requestId, copy);
    } catch (err) {
      console.error("[/api/payment/initiate] dev generateCopy error:", err);
      storeResult(requestId, "[Erreur de génération — vérifiez votre clé OpenAI]");
    }
    return NextResponse.json({
      checkoutUrl: `${appUrl}/success?token=${requestId}`,
      token: requestId,
    });
  }
  ...
  try {
    const paydunyaRes = await fetch(`${baseUrl}/checkout-invoice/create`, {
      ...
      body: JSON.stringify({
        ...
        actions: {
          cancel_url: appUrl,
          return_url: `${appUrl}/success?token=${requestId}`,
    ...
    const result = await paydunyaRes.json() as { response_code?: string; description?: string; checkout_url?: string };
